...
public int scoresAverage(int[] scores) {
    int averageFirst = averageFinder(scores, 0, scores.length / 2);
    int averageSecond = averageFinder(scores, scores.length / 2, scores.length);
    return Math.max(averageFirst, averageSecond);
}

public int averageFinder(int[] x, int start, int end) {
    int sum = 0;
